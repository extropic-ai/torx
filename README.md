...
The PyPI distribution is `extro-torx`; the Python import remains `torx`.
...
A gallery of runnable tutorial notebooks walks through Torx end to end, from a first one-gate circuit to random walks on graphs, chemical reaction networks, discrete diffusion, trained stochastic graph networks, and directed factor graphs. Browse them rendered in the [example gallery](https://docs.torx.ai/en/latest/examples.html), or run the sources under [`examples/`](examples/).
