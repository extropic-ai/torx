...
class PditShift(AbstractSinglePditGate):
    ...
    def get_matrix(self) -> Float[Array, "d d"]:
        ...
        return (1 - self.prob) * identity + self.prob * cyclic
...
class PditSWAP(AbstractMultiPditGate):
    ...
    def get_matrix(self) -> Float[Array, "d2 d2"]:
        """See `AbstractPGate.get_matrix` for documentation."""
        d = self.dims[0]
        d2 = d * d
        ...
        identity = jnp.eye(d2)
        return (1 - self.prob) * identity + self.prob * swap
...
class PditCycle(AbstractSinglePditGate):
    ...
    def __check_init__(self):
        ...
        if theta.shape != (2,):
            raise ValueError(
                f"PditCycle requires theta with shape (2,), got {theta.shape}"
            )
    ...
    def get_matrix(self) -> Float[Array, "d d"]:
        """See `AbstractPGate.get_matrix` for documentation."""
        d = self.dims[0]
        p = self.probs  # [p_identity, p_forward, p_backward]
        identity = jnp.eye(d)
...
PditShift.__init__.__doc__ = """See [`torx.AbstractSinglePditGate.__init__`][]."""
PditSWAP.__init__.__doc__ = """See [`torx.AbstractMultiPditGate.__init__`][]."""
PditCycle.__init__.__doc__ = """See [`torx.AbstractSinglePditGate.__init__`][]."""
